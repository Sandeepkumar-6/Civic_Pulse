...
(async () => {
  ...
  console.log('body text=', (await page.locator('body').textContent()).slice(0, 1000));

  const payload = await page.evaluate(async () => {
    const res = await fetch('/api/reports/map?west=73.79&south=18.54&east=73.83&north=18.58', { credentials: 'include' });
    const text = await res.text();
    return { status: res.status, body: text.slice(0, 800) };
  });
  console.log('map endpoint payload=', JSON.stringify(payload));
